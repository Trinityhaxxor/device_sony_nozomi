...
static void eos_power_set_interactive(struct power_module *module, int on)
{
...
{
    eos_power_set_interactive(NULL, 1);
...
static int boostpulse_open(struct eos_power_module *eos)
{
    char buf[80];

    pthread_mutex_lock(&eos->lock);

    if (eos->boostpulse_fd < 0) {
        if (get_scaling_governor() < 0) {
            ALOGE("Can't read scaling governor.");
            eos->boostpulse_warned = 1;
        } else {
            if (strncmp(governor, "ondemand", 8) == 0)
                eos->boostpulse_fd = open(BOOSTPULSE_ONDEMAND, O_WRONLY);
            else if (strncmp(governor, "interactive", 11) == 0)
                eos->boostpulse_fd = open(BOOSTPULSE_INTERACTIVE, O_WRONLY);

            if (eos->boostpulse_fd < 0 && !eos->boostpulse_warned) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGV("Error opening boostpulse: %s\n", buf);
                eos->boostpulse_warned = 1;
            } else if (eos->boostpulse_fd > 0) {
                configure_governor();
    ...
    pthread_mutex_unlock(&eos->lock);
    return eos->boostpulse_fd;
}

static void eos_power_hint(struct power_module *module, power_hint_t hint,
                            void *data)
{
    struct eos_power_module *eos = (struct eos_power_module *) module;
    char buf[80];
    int len;
    int duration = 1;
    ...
    case POWER_HINT_CPU_BOOST:
        if (boostpulse_open(eos) >= 0) {
            if (data != NULL)
                duration = (int) data;

            snprintf(buf, sizeof(buf), "%d", duration);
            len = write(eos->boostpulse_fd, buf, strlen(buf));
            ...
            if (len < 0) {
                ...
                pthread_mutex_lock(&eos->lock);
                close(eos->boostpulse_fd);
                eos->boostpulse_fd = -1;
                eos->boostpulse_warned = 0;
                pthread_mutex_unlock(&eos->lock);
            }
        }
        break;
...
static void eos_power_init(struct power_module *module)
{
...
struct eos_power_module HAL_MODULE_INFO_SYM = {
    base: {
        common: {
       ...
       init: eos_power_init,
       setInteractive: eos_power_set_interactive,
       powerHint: eos_power_hint,
    },
